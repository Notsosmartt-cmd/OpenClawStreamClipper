...
def _clip_file_for(title: str) -> Path | None:
    tnorm = occ._norm(title)
    for f in CLIPS.glob("*.mp4"):
        stem = f.stem
        if stem.endswith(" (B)") or stem.endswith(" (Short)"):
            continue
        if occ._title_match(occ._norm(stem), tnorm):
            return f
    return None
...
def select_stories(vod_stems: list[str], max_per_vod: int, budget_s: float,
    ...
    # round-robin: everyone's #1 first (per-VOD guarantee), then #2s, under budget
    picked: list[dict] = []
    rank = 0
    while True:
        added = False
...
def main() -> int:
    ...
    if not stories:
        ...
        return 1
    log(f"{len(stories)} stories: " + " | ".join(f"{s['streamer']}:{s['title'][:35]}" for s in stories))

    use_vo = not args.no_vo
    with tempfile.TemporaryDirectory(prefix="news_") as td:
        work = Path(td)
        pieces: list[Path] = []
        intro = render_intro(stories, work, use_vo)
        if intro:
            pieces.append(intro)
        for i, s in enumerate(stories):
            log(f"story {i + 1}/{len(stories)}: {s['streamer']} — {s['title'][:50]}")
            seg = render_story(s, i, work, args.story_len, use_vo)
            if seg:
                pieces.append(seg)
        if len(pieces) < 2:
            log("not enough rendered pieces — aborting")
            return 1
        date_name = time.strftime("%m-%d-%Y")
        out = Path(args.out) if args.out else CLIPS / f"STREAMERS UPDATE {date_name}.mp4"
        if not concat(pieces, out, work):
            return 1
