...
# Speed Wave 2 P0.2 (plan-serving-stack-2026-07): with CLIP_PASSB_DUMP_PROMPTS=1, also
# append each assembled moment prompt to <work>/passb_prompts.jsonl so bench_serving.py can
# replay REAL Pass-B prompts (chunk N's prompt embeds N-1/N-2 LLM outputs, so an offline
...
If nothing stands out at all, respond: {{"moments": []}}"""
    ...
    # Speed Wave 2 P0.2: dump the real assembled prompt for offline bench replay.
    if _PASSB_DUMP_PROMPTS:
        try:
            import json as _json
            with open(f"{TEMP_DIR}/passb_prompts.jsonl", "a", encoding="utf-8") as _pf:
                _pf.write(_json.dumps({
                    "idx": chunk_count, "kind": "moment",
                    "start": int(chunk_start), "end": int(chunk_end),
                    "seg_type": seg_type, "prompt": prompt,
                }) + "\n")
        except Exception:
            pass
