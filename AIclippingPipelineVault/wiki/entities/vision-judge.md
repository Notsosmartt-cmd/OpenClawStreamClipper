...
> `swiss_tournament()` now dispatches each round's independent pairings through a `ThreadPoolExecutor` (`workers` param; `JUDGE_WORKERS` env / `judge.json:workers`, default 2), folding results sequentially. Rounds still re-rank between themselves, and pairings are fixed from the round-start order so each item plays once per round — the parallel result is **identical to the serial path** (unit-verified), just ~2× faster. Note Stage 5.5 only **re-orders a set that renders in full** ([[concepts/clip-quality-remediation-2026-06]] Fix 2): set `judge.json:enabled=false` to skip it entirely if the re-rank isn't worth even the halved cost. **To decide that, watch the new `[JUDGE] rank churn: X/N clips moved; #1 changed|unchanged` log line** (2026-06-06) — if churn stays ~0 across runs the judge is just re-deriving Pass C order. Quick per-run tuning via `JUDGE_MAX_COMPARISONS` / `JUDGE_FRAMES_PER_CLIP` / `JUDGE_SHORTLIST_MAX` env (override `judge.json`).
