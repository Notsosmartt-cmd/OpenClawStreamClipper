...
| **Clip toggles (checkboxes)** | Captions, hook caption, `chk-enable-thinking` (model thinking, default off — BUG 67), `chk-companion-shorts` (`CLIP_COMPANION_SHORTS`), **`chk-ab-variants` → `CLIP_AB_VARIANTS=2` (2026-07-10, classic A/B variant clips; needs Style profiles ON for varied SFX/visual)**, **`chk-post-kit` → `CLIP_POST_KIT` (per-platform `.post.json`, no hashtags)**. All default off; each JS checkbox → request field → `spawn_pipeline`/`pipeline_env` env var (both docker + bare-metal paths). See [[concepts/plan-captions-and-ab-variants-2026-07]]. |
