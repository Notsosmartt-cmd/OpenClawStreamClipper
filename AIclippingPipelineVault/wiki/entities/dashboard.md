...
|---|---|---|
| Framing mode | `CLIP_FRAMING` + `CLIP_CAMERA_PAN` + `CLIP_FRAME_MODE` | **ONE dropdown, 4 modes** (re-consolidated 2026-07-19 after a duplicate "Frame fit" select briefly appeared in Clip Controls): `auto` (default — per clip by segment type: irl/just_chatting → full-bleed, gaming/reaction → blur-fill) · `blur_fill` (letterbox) · `fill` (full-bleed crop to action) · `camera_pan` (face track). One selection derives all three env vars via `frameModeFromFraming()`; **camera_pan maps to frame_mode=blur** so the static crop can't fight the moving one (stage7 guards this independently). First consolidated 2026-05-02 (dropdown + separate checkbox that had to agree manually) — same failure mode both times. |
