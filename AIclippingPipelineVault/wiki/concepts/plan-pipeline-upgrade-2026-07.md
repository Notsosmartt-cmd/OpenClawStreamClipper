...
> [!success] Phase 1 LIVE WIRING landed (2026-07-04) — anomaly lane in Stage 4, `CLIP_ANOMALY_LANE` (default OFF)
> Wired into `stage4_moments.py` right before the Pass-C dedup/sort (after keyword+llm merge into `all_moments`). **Cheap-by-design:** the reaction signal is the librosa **`crowd_response` ALREADY computed in Stage 2** (no CLAP-over-VOD cost — that was the cost gotcha); `event_timeline.build_timeline(words, crowd_events)` → `anomaly_propose.propose(keyword_fn=max Pass-A score in window, verify_fn=verify_via_lmstudio)`; the few-shot LLM verifier runs only on the **top-6** survivors. Survivors append to `all_moments` as `source="anomaly"` / `src="ANOMALY"` moments (a **superset of the keyword-moment schema** + real `clip_start/end/duration`), boost-only into the same Pass C pool. Entirely gated + `try/except` failure-soft. **Flag-OFF = byte-identical by construction** (only a comment sits outside the `if`; verified: keyword & llm moments already have disjoint field sets, so Pass C `.get`s all optionals). **Flag-ON validation run in progress via the harness** (does Stage 4 stay healthy + do `src=ANOMALY` moments appear on rakai?). Motion (Stage-5 visual) + true CLAP semantic events are a documented v2 enhancement.
...
**7.1 Draft-annotation loop → measurable detection reliability (~½ day).** Reliability of effect/cue detection can't be *asserted*, only *measured* — and measurement needs labels (only **1/35** clips has a `.notes.json`). Owner-cheap path: a `--draft-notes` mode in `clip_forensics` writes a **pre-filled `.notes.json`** from its own detections (SFX events, music spans, censor, cuts) for each clip; the owner CORRECTS drafts (delete wrong, add missed — minutes per clip) instead of annotating from scratch. `_score_against_notes` (already built) then yields real **per-detector precision/recall across the corpus**, which calibrates the CLAP per-label thresholds (E1) with data instead of the current single-clip guesses. This is THE answer to "can it reliably detect" — after 7.1 we'll have the number.

**7.2 Caption-language style learning (~1 day).** The pieces exist: `caption_ocr` extracts burned-in caption TEXT (verified: real text @ 13 wps), and `stage6_vision` + `config/hook_templates.json` already inject hook-text templates into generated clips. Missing middle: a `caption_style.py` distiller that runs OCR text from ALL reference clips through the local LLM once → emits `config/caption_style.json` — casing habits (lowercase-only vs CAPS), emoji/slang lexicon, hook phrasings, length distribution, per-category tone — consumed two ways: (a) **few-shot examples in the Stage 6 hook/title prompt** ("write the hook in this voice: <5 real competitor captions>"), (b) **grown `hook_templates.json` entries**. Caveat: OCR learns *language* style reliably; *visual* styling (font/color/position) only roughly (box geometry); emoji partially (rendered images sometimes unreadable).

**7.3 Transcript-value classifier (~½ day).** Per reference clip, score the transcript STANDALONE: words/sec, Pass-A keyword score, and an LLM judgment ("does this text alone justify a clip?") → classify **transcript-carried vs reaction/visual-carried**. Answers "does the transcription have value" per clip, and — the strategic payoff — **labels which corpus clips are the anomaly-lane class** (reaction-carried), giving the lane a ground-truth eval set beyond the 2 hand-analyzed cases.
