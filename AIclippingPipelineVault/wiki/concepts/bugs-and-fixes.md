...
**Root cause A — `google/gemma-4-26b-a4b-qat` is a NO-GO for this pipeline (the important one).** The owner had switched `config/models.json` `text_model`/`vision_model` to gemma-4-26b (an uncommitted local experiment — the "Lever 2 / fit-more-on-the-fast-card" idea). gemma-4-26b is a **reasoning model that ignores the pipeline's `/no_think` directive**: it emits a `<think>` block that consumes the ENTIRE token budget and never produces the JSON answer. Log signature, **379 times** in one run: `LLM returned empty content: finish=length, reasoning_tokens=7997, total_tokens=8000`. Many chunks → empty content → 3 retries (~6 min each) → `LLM call failed` (71×); it doesn't cleanly fail so much as **grind ~6 min/chunk**. **Measured timeline (from the log):** the batch launched at local 03:00:10; VOD 1 **Lacy** (a 6.8 h VOD, ~61 Stage-4 chunks) took **5 h 52 m (21155 s)** and DID complete with 20 clips — Stage 4 alone ran ~4.5 h (03:46→08:17), finding a few moments per chunk between failures; its vision stage also failed → transcript-score fallback. (Context: even a healthy Lacy run is ~5.5 h because it's the 6.8 h monster VOD — realtime ratio 0.82; the gemma/double-launch overhead added ~20 min, not hours.) VOD 2 **Jynxzi** entered Stage 4 at 09:31 and was still grinding chunk ~16 (mix of 0-1 moments + empty-content failures) 1.5 h later at 11:03 when the PC hard-crashed — it never finished. VOD 3 never started. So "8 h" = Lacy's legit ~5.9 h + Jynxzi's 1.5 h partial + the ~46 min Stage 1-3 each, doubled-up by the two contending processes.
...
- **FIX SHIPPED (2026-07-09):** the launch guard was per-dashboard-process only — the routes DO check `is_pipeline_running()` (return 409), but it read only the in-memory `_state.pipeline_process`, which is None in a SECOND or RESTARTED dashboard, so the owner's "start → stop → restart-to-change-a-setting → play" launched a duplicate. Made `is_pipeline_running()` **cross-process**: it now also checks the on-disk `pipeline.pid` marker (`_marker_pid()` + `_pid_alive()`; a `pipeline.done` at/after the pid marker = finished). A second/restarted dashboard now sees the running pipeline and its launch returns 409. Also `api_stop` → `stop_running_pipeline()` kills the marker pid (`taskkill /F /T`) even without an in-memory handle, so a restarted dashboard can still stop the run it didn't launch. Logic unit-tested (live-pid→running, done→finished, dead-pid→not). `dashboard/pipeline_runner.py` + `pipeline_routes.py`.
