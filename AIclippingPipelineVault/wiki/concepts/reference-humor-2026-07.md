...
- **Chat region (ROI): AUTO-DETECT IS THE DEFAULT** (owner decision 2026-07-03 — "I don't know where chat will be"). Method: sample ~10–20 frames spread across the VOD → EasyOCR *detection* pass (text boxes only, no recognition — fast) → build a heatmap of where **many small text lines persistently cluster in a stable region with changing content**. That signature separates chat (small, dense, stable position, always-changing text) from burned-in captions (large, centered, transient) and HUD elements (static text that never changes). The winning cluster = the chat ROI. **Bonus: this doubles as the "does this VOD even have chat?" test** — no persistent small-text cluster → no CHAT track, failure-soft, zero config (exactly right for the owner's mixed corpus of stream recordings + YouTube MP4s). An optional per-channel override (the `streamer_prompts.json` pattern) exists only for the rare case auto-detect picks wrong.
