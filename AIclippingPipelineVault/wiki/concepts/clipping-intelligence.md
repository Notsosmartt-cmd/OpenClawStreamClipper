...
| **Selection axes** (A arc, B reaction) | `arc_completeness.py` / `reaction_signals.py` | each a bounded, failure-soft `×mult` (A may demote to 0.85; B boost-only ≤1.10) accumulated into one `axis_mult` product |
...
> Plan A (arc-completeness) and Plan B (reaction-worthy) are live as Pass C pre-signals; C/E are planned.
