...
>
> **The 0.6 floor is VOD-dependent (2026-06-06, Lacy run `20260606_235356`).** A1 found 5 genuine arcs (irony "scored a penalty", contradiction "best goalkeeper", fulfillment "nando's in LA", exposure "I'll show you later") but **none reached the final 12 and the guarantee did NOT fire** — on this longer, content-dense VOD the abundant strong Pass B `setup_external_contradiction`/`storytelling_arc` moments out-scored the dedicated arcs (weakest winner raw 1.36 → an arc needed final ≥0.82 to clear 0.6×). This is the floor working *correctly* (don't force a weak arc over a strong clip), but it means on rich VODs the cross-chunk arcs stay invisible — and **Fix 3 arc-stitch can't trigger either**, since it only stitches an arc that's already in the selection. To showcase Lacy-style arcs, **lower `CLIP_ARC_GUARANTEE_MIN_RATIO` to ~0.45** so the top arc gets a slot, then **`CLIP_ARC_STITCH=1`** to render it as a setup→payoff stitch. The dashboard **"Stitch setup→payoff arcs"** checkbox ([[entities/dashboard]]) does both in one toggle (`config_io.py` sets `CLIP_ARC_STITCH=true` *and* the ratio to `0.45`). See [[concepts/detection-improvements-plan]] Fix 3.
