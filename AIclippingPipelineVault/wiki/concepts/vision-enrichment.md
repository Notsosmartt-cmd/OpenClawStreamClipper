...
- **Grounding is field-aware (Fix 1A, 2026-06-06)**: `title`/`hook` are intentionally creative, so they run the Tier-1 denylist + Phase-2.4d hard-event check ONLY (overlap gate off, no *weighted-mean* judge) — blocking fabricated events while allowing punchy phrasing. `description` keeps the full two-tier cascade. See [[concepts/clip-quality-remediation-2026-06]] Fix 1.
- **Caption quality gate (P1.2/P1.4, 2026-07-10)** — the creative fields no longer skip ALL semantic checks. After the Tier-1 pass, `_caption_gate` runs a deterministic AI-tell linter (`caption_lint.py` — Title Case / scare-quotes / "The 'X' Y" / clickbait / em-dash / hashtag; `CLIP_CAPTION_LINT=1`) then a `grounding.caption_judge` fidelity+voice call pinned to the loaded Stage-6 model (`CLIP_CAPTION_JUDGE=1`, fidelity < 6 fails). A fail reuses the SAME regenerate-once → null-to-baseline machinery. Also **P1.1**: the clip-window transcript feeding both generation and grounding is now the FULL window (`[:4000]`, was `[:500]` ≈ first ~25 s — long clips were titled from a fraction of their words). A voice-contract prompt block (`CLIP_CAPTION_VOICE=1`) bans the AI-tells at generation time. Live-validated: on-topic caption fidelity 10, off-topic 0. See [[concepts/plan-captions-and-ab-variants-2026-07]].
