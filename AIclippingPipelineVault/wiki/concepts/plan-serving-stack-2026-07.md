...
| **Moment-parallel (Speed #5 cut-over 2)** | ✅ WIRED (flag `CLIP_PASSB_MOMENT_WORKERS`, default 1=off) — **gate A/B running** | Owner approved 2026-07-09 (accepted the variance-gate framing over byte-identity). `stage4_moments.py`: post-call block extracted to `_process_moment_response()` (serial path routes through it, behavior-identical); parallel mode = eager card/summary at submit (two-phase prior-context from precomputed cards; forces card precompute on) + ≤N in-flight with 1-chunk-lag drain + post-loop drain. Gate: `moment_parallel_ab.py` — Stage-4 wall ≥1.4× AND clip overlap ≥5/10 ±20 s vs the serial arm. PROMOTE default-on or DELETE on result |
