...
| **Moment-parallel (Speed #5 cut-over 2)** | 🔴 REJECTED by the gate + DELETED (2026-07-09) | Wired (owner-approved variance-gate framing), then the A/B REJECTED on BOTH gates: **Stage-4 speedup 1.15×** (1107→959 s) vs ≥1.4× required, **clip overlap 4/10 ±20 s** vs ≥5/10. Code reverted same day (no default-off zombie); gate driver `moment_parallel_ab.py` + verdict `moment_ab_20260709_141429.json` kept as forensics. **This closes the LLM floor completely**: even request-level concurrency — the last software lever — returns ~1.15× on the cross-vendor split (consistent with card-parallel ~4% and the vision-slot noise). Serving co-batching barely amortizes the coordination cost. Stage-4 speed now has exactly ONE lever: hardware (single big card) or a model-tier change |
