...
| **Moment-parallel (Speed #5 cut-over 2)** | 🟡 RE-INSTATED, default-off, **pending owner spot-check** (2026-07-09) | Flag `CLIP_PASSB_MOMENT_WORKERS` (default 1=off). First A/B gave **1.15×** Stage-4 (1107→959 s) + **4/10** overlap; owner LOWERED the speed bar **1.4×→1.1×** ("15% is worth it on long VODs like Lacy — ~30 min/run") and reclassified overlap as ADVISORY (a different-but-good draw is fine; overlap ≠ quality). So it PASSES the speed gate; the remaining gate is the owner reviewing the parallel run's clips (6 differ from serial: the two 'Freestyle' hype clips, 'Principal Addresses…', 'Waiting for the Principal', 'Caught Lying About His Sister', 'Samurai Slicer Diss'). **Promote to default-on on owner OK; the code stays flag-gated meanwhile.** Value is concentrated on long VODs (Lacy Stage-4 ~3 h → ~30 min saved; typical 2-3 h VOD → ~3 min). `moment_parallel_ab.py` speed-gate now 1.1×, overlap advisory |
