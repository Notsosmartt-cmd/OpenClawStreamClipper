...
| 3 | Richer joint prompts at 5.5/6 (timeline into the judge) | **Built 2026-07-08, flag-gated OFF** | `CLIP_JUDGE_TIMELINE` (default off): `stage5_5_judge` builds an event timeline (words + Stage-2 `crowd_response` audio) per clip and `vlm_judge.compare_pair` injects it alongside the frames via a new `timeline_fn`. Failure-soft. Activates in Activation-Wave Run 2 ([[concepts/plan-activation-wave-2026-07]]). Bounded: re-ranks the shortlist only; full value needs CLAP-live (named events vs one energy dial) |
