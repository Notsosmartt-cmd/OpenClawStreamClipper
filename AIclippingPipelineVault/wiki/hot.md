...
- **Clip-forensics research DONE → ready to build (2026-06-13)** — [[concepts/clip-forensics-research-2026-06]] has the verified tool matrix + commercial-license flags + architecture + a copy-paste **engineering prompt**. Commercial-safe stack: PANNs CNN14 (MIT) + LAION-CLAP + inaSpeechSegmenter (MIT) + better-profanity (MIT) + EasyOCR/PySceneDetect; **Essentia dropped (AGPL/NC)**, **Demucs weights offline-only**, **MS-CLAP license unconfirmed**. Plan [[concepts/plan-clip-forensics]] now in-progress; `reference_clips/` scaffolded (curate clips → run phase-1 `audio_sense`+`clip_forensics`). Perception gap: [[concepts/model-senses]].
