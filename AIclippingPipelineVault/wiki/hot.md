...
- **Clip-forensics + semantic-sensing plan filed (2026-06-13)** — [[concepts/plan-clip-forensics]] has a full research-handoff brief (CLAP/PANNs audio sensing → decompose curated reference clips into style profiles; upgrades `audio_events.py`, feeds the anomaly proposer + SFX placement). Perception gap documented in [[concepts/model-senses]]. `reference_clips/` scaffolded for hand-curated clips (no yt-dlp yet). Ready to hand to a research agent.
